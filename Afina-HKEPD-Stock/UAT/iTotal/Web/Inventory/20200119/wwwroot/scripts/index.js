...
function renderSbPopups() {
    switcherPopup = new ej.popups.Popup(document.getElementById('sb-switcher-popup'), {
        relateTo: document.querySelector('.sb-user-text'),
        position: { X: 'left' },
...
function onsearchInputChange(e) {
    ...
    if (searchVal.length) {
        ...
        var controlsAccess = [];
        for (var i = 0, ctls = controls; i < controls.length; i++) {
            var cont = ctls[i];
        ...
        for (var k = 1; k < controls.length; k++) {
            var curComponent = controls[k].component;
            var previd = controlCollection[curComponent];
            if (previd) {
                controls[k].sortId = previd;
            } else {
                ++count;
                controlCollection[curComponent] = count;
                controls[k].sortId = count;
            }
        }
        if (!searchListView) {
...
function getMenuList() {
    ...
    var tmp = getSampleData();
    //var tmp = window.samplesList;
    var tempList = ej.base.extend([], tmp);
    var sampleList = [];
...
function renderLeftPaneComponents() {
    samplesTreeList = getTreeviewList(menuList);
...
function getTreeviewList(list) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        controlSampleData[list[i].directory] = getSampleswithHidden(list[i].samples);
    }
    return tempList;
}

function getSampleswithHidden(samples) {
    var tempSamples = [];
    ...
    if (path) {
        if (curHashCollection.indexOf(path) === -1) {
            ...
            if (arg.data) {
                ...
                if (arg.data.dir === 'ReportServer') {
                    var url = getReporturl();
                    window.open(url + 'ReportViewer/' + arg.data.url + '/Preview', '_blank');
...
function onDataSourceLoad(node, subNode, control, sample, sampleName) {
    ...
    title.innerHTML = node.name + ' · ' + subNode.url ;
